...
class DFS(Solution):
    def solve(self, numCourses: int, prerequisites: list[list[int]]):
        ...
        def check(pre):
            ...
            for idx, (cur_, pre_) in enumerate(prerequisites):
                if not checked[idx] and cur_ == pre:
                    checked[idx] = True
                    path.append(cur_)
                    check(pre_)
                    path.pop()

        for idx, (cur, pre) in enumerate(prerequisites):
...
# NOTE while edges list together with adjacency list and adjacency matrix are three main ways to represent a graph, it's not commonly used to solve graph problems.
# One main reason is that to perform a common operation like getting all descendants of a given vertex it takes O(len(edgesList)). Hence, we can first build an adjacency list from the edges list which takes O(1) to perform such operation.
...
class DFSOpt(Solution):
    def solve(self, numCourses: int, prerequisites: list[list[int]]):
        ...
        vis = []
        adjace = {i : [] for i in range(numCourses)}

        for course, pre in prerequisites:
            adjace[course].append(pre)
        ...
        def check(course):
            ...
            for pre in adjace[course]:
                if not check(pre):
                    return False
            vis.pop()
            # the path from courses are all checked, and it can be cleared
            adjace[course] = []

            return True


        for course in adjace:
            if not check(course):
                return False
...
class DFSOpt2(Solution):
    def solve(self, numCourses: int, prerequisites: list[list[int]]):
        ...
        State = Enum('State', ["NotVisited", "Processing", "Completed"])
        state = [State.NotVisited] * numCourses

        def has_cycle(course_i):
            if state[course_i] == State.Completed:
                return False

            if state[course_i] == State.Processing:
                return True

            state[course_i] = State.Processing

            for i in adjace[course_i]:
                if has_cycle(i):
                    return True

            state[course_i] = State.Completed
...
# modified from DFS2, change the record status to stack
# NOTE in essence it is same as the DFSOPT
# NOTE for this we need a visited and a stack to record the status
# so DFS2 which uses 0, -1, 1 to record the status is better
class DFSOPT3(Solution):
    def solve(self, numCourses: int, prerequisites: list[list[int]]):
        ...
        def has_cycle(course_i, stack):
            self._counter()
            if course_i in stack:
                return True
            if course_i in visited:
                return False

            stack.append(course_i)

            for i in adjace[course_i]:
                if has_cycle(i, stack):
                    return True
            stack.pop()
            visited.add(course_i)
...
# NOTE this is actually BFS
# 1. initially, we push all the nodes without precedents into the Q
# 2. we find all the children of these nodes, and we cut the arrow pointing to them
# repeat 1 and 2 until all nodes are visited
# if the node number is not equal to the total, then there must be loop
...
class TopSort(Solution):
    def solve(self, numCourses: int, prerequisites: list[list[int]]):
        ...
        Q = [i for i in range(numCourses) if in_degrees[i] == 0]
